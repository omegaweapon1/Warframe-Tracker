# Version 1.07
# - Increased font size of all visible text elements including date, headers, subheaders, buttons, list items, and submenu elements.
...
class TaskTrackerApp:
    ...
    def create_task_frames(self):
        ...
        # We cannot set font on LabelFrame directly, so set a bold label above instead
    ...
    def populate_task_columns(self):
        ...
        def add_tasks_with_separators(parent, tasks_list):
            ...
            for task in tasks_list:
                if task == "---":
                    # Only add separator if last was not separator
        ...
        # Weekly Tasks: with subheaders bigger font
        for section in WEEKLY_TASKS:
            label = ttk.Label(self.weekly_col, text=section, font=self.font_sub_header)
            label.pack(anchor="w", padx=10, pady=(5, 0))
            add_tasks_with_separators(self.weekly_col, WEEKLY_TASKS[section])

        # Add requested line breaks in Weekly Quests between specific tasks
        # Find the positions of the tasks to insert a separator after them
        quest_tasks = WEEKLY_TASKS["Quests"]
        # Use children widgets list to insert separators after those checkbuttons
        children = self.weekly_col.winfo_children()
        # Find index of checkbuttons matching the tasks to add separator after
        # But careful, label widgets and separators also in children

        # We pack tasks sequentially, so their order in children is after the label for the section
        # We add separator only if task is visible and not completed, so index varies

        # To handle this accurately, let's destroy weekly_col children and re-add all with manual separators for the 3 places

        # Rebuild weekly quests with separators as needed
        for widget in self.weekly_col.winfo_children():
            widget.destroy()
        ...
        for task in WEEKLY_TASKS["Quests"]:
            if task in ["Archon Hunt", "Netracell", "SP Circuit"]:
                # add separator before next task except before first task
                # but only if previous was not separator and task is visible
                if not last_was_separator:
            ...
            if self.visibility_settings.get(task, tk.BooleanVar(value=False)).get() and not self.checked_tasks.get(task, tk.BooleanVar(value=True)).get():
                ...
                # If task is hidden, no cb, so last_was_separator unchanged
                pass
    ...
    def populate_timer_rows(self):
        ...
        # Timer label font increase via style
        style = ttk.Style()
    ...
    def complete_tasks(self):
        # Only complete tasks that are currently visible (opted-in & not completed)
        for task in list(self.checked_tasks.keys()):
            if self.visibility_settings.get(task, tk.BooleanVar(value=False)).get() and self.checked_tasks.get(task, tk.BooleanVar(value=False)).get():
                # Mark completed, so hidden from main until reset
    ...
    def open_settings(self):
        ...
        # Font increase for label frames: ttk.LabelFrame does not support font directly
        # So add label inside with bigger font or style
        for frame in (daily_col, weekly_col):
        ...
        for section in DAILY_TASKS:
            ...
            for task in DAILY_TASKS[section]:
                if task == "---":
                    ...
                    continue
                cb = ttk.Checkbutton(daily_col, text=task, variable=self.visibility_settings[task], command=self.refresh_task_lists, style="Task.TCheckbutton")
                cb.pack(anchor="w", padx=20)
        ...
        for section in WEEKLY_TASKS:
            ...
            for task in WEEKLY_TASKS[section]:
                cb = ttk.Checkbutton(weekly_col, text=task, variable=self.visibility_settings[task], command=self.refresh_task_lists, style="Task.TCheckbutton")
                cb.pack(anchor="w", padx=20)
                if section == "Quests":
                    if task == "Archon Hunt":
