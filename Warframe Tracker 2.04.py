# Version 2.04
# - Automatic daily reset at 00:00 UTC unchecks daily tasks and refreshes UI
# - Automatic weekly reset at 00:00 UTC Sunday unchecks weekly tasks and refreshes UI
# - Added "Simulate Day" and "Simulate Week" buttons for manual testing
# - Preserves all features from 2.02 including submenu formatting and window position memory
# - Fixed missing open_settings method error
...
class TaskTrackerApp:
    def __init__(self, root):
        ...
        # Run reset check on startup immediately
        self.check_for_reset()
        # Schedule automatic reset check every minute
    ...
    def update_timer_labels(self):
        ...
        self.timer_labels["Coda Weapon Reset"].set(f"Coda Weapon Reset (Next Reset in {delta_coda.days} Days, {delta_coda.seconds // 3600} Hrs)")
        self.timer_labels["Tenet Weapon Reset"].set(f"Tenet Weapon Reset (Next Reset in {delta_tenet.days} Days, {delta_tenet.seconds // 3600} Hrs)")
        self.timer_labels["Baro Ki'Teer"].set("Baro Ki'Teer (Check In-Game)")
    ...
    def reset_tasks(self):
        # Uncheck all visible daily and weekly tasks (only those opted in)
    ...
    def complete_tasks(self):
        # Check all currently visible tasks and mark as completed (checked)
        for task in self.checked_tasks:
            if self.visibility_settings.get(task, tk.BooleanVar(value=False)).get():
    ...
    def simulate_day_reset(self):
        # Simulate daily reset at 00:00 UTC Monday (daily tasks unchecked)
        for task in [t for s in DAILY_TASKS.values() for t in s if t != "---"]:
            if task in self.checked_tasks:
                self.checked_tasks[task].set(False)
        self.refresh_task_lists()
    ...
    def simulate_week_reset(self):
        # Simulate weekly reset at 00:00 UTC Sunday (both daily and weekly tasks unchecked)
        for task in [t for s in DAILY_TASKS.values() for t in s if t != "---"]:
            if task in self.checked_tasks:
                self.checked_tasks[task].set(False)
        for task in [t for s in WEEKLY_TASKS.values() for t in s]:
