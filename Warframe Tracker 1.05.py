# Warframe Task Tracker v1.05
# 1.0 - Starting point. GUI with today's date at the top, with time to daily reset at 0:00 UTC
#       Column 1 is daily tasks header with quests, reputation, and vendor as sub headers
#       Column 2 is weekly tasks header with vendors and quests as sub headers
#       Custom timer section on bottom to track Tenet, Coda, and Baro
#       Gear sub menu currently not working as intended
#       Complete checked button forced to bottom ribbon, marks current selections and complete and removes them from the list until the daily/weekly/custom reset
# 1.01 - Successfully fixed gear sub menu, it now works as an opt-in filter
#        Only tasks opted in can appear on the main list or timers
#        Completion checkboxes only work on currently visible tasks
#        Complete tasks hidden until next relevant reset
# 1.02 - Changed main header font size and boldness
# 1.03 - Teshin under Reputation renamed to Conclave to prevent issues with Teshin under Vendors
# 1.04 - When marking items complete, remove redundant line breaks
# 1.05 - Added line breaks between Archon Hunt → Deep Archimedea, Netracell → Circuit, SP Circuit → Hex Calendar
...
class TaskTrackerApp:
    ...
    def create_header(self):
        ...
        date_label = ttk.Label(header_frame, text=self.get_date_string(), font=("Segoe UI", 11, "bold"))
    ...
    def create_task_frames(self):
        ...
        self.daily_col = ttk.LabelFrame(content_frame, text="Daily Tasks")
        self.weekly_col = ttk.LabelFrame(content_frame, text="Weekly Tasks")
    ...
    def populate_task_columns(self):
        ...
        def add_tasks_with_separators(parent_frame, tasks):
            ...
            for task in tasks:
                ...
                elif self.visibility_settings[task].get() and not self.checked_tasks[task].get():
