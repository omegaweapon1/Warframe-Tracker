# Warframe Task Tracker v1.0
# - Moved "Complete Checked" button to a fixed bottom ribbon that stays visible and unmoved
# - Ensured complete button is outside the scrollable area to prevent it from shifting or disappearing
...
DAILY_TASKS = {
    "Quests": [
        ...
        "KIM",
        "Syndicate Missions",
        "Steel Path Incursions"  # Added here
    ],
    "Reputation": [
        "Ostron",
        "Quills",
...
class TaskTrackerApp:
    def __init__(self, root):
        ...
        # Main container frame to hold scrollable + fixed button frame
            ...
            for section in task_group:
                for task in task_group[section]:
                    if task != "---":
                        ...
                        self.checked_tasks[task] = tk.BooleanVar()
        ...
        self.settings_window = None  # Track settings window reference
        ...
        for section in WEEKLY_TASKS:
            ...
            for i, task in enumerate(WEEKLY_TASKS[section]):
                if self.visibility_settings[task].get():
                    ...
                    cb.pack(anchor="w", padx=20)
                # Add line breaks between specific tasks
                if section == "Quests":
                    if task == "Archon Hunt":
    ...
    def create_bottom_ribbon(self):
        # Fixed frame at bottom for Complete button
    ...
    def complete_tasks(self):
        for task, var in self.checked_tasks.items():
            if var.get():
                self.visibility_settings[task].set(False)
                var.set(False)
    ...
    def open_settings(self):
        ...
        ttk.Label(self.settings_window, text="Show/Hide Tasks", font=("Segoe UI", 10, "bold")).pack(pady=5)
