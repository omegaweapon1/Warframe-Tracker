...
Version 1.03
Changes:
- Changed 'Teshin' under Reputation to 'Conclave' to avoid name conflict with Teshin under Vendors.
- Maintained header font size and boldness changes from 1.02.
"""
...
DAILY_TASKS = {
    ...
    "Reputation": [
        ...
        "---",
        "Cephalon Simaris",
        "Conclave"  # Changed from Teshin to Conclave here
    ],
    "Vendor": [
        "Acrithis - Arcanes"
    ]
...
class TaskTrackerApp:
    ...
    def create_header(self):
        ...
        # Larger, bold font for main header per version 1.02 changes
        date_label = ttk.Label(header_frame, text=self.get_date_string(), font=("Segoe UI", 12, "bold"))
    ...
    def create_task_frames(self):
        ...
        # Larger bold labels for main columns
        self.daily_header = ttk.Label(content_frame, text="Daily Tasks", font=("Segoe UI", 12, "bold"))
        self.weekly_header = ttk.Label(content_frame, text="Weekly Tasks", font=("Segoe UI", 12, "bold"))

        self.daily_header.grid(row=0, column=0, padx=10, sticky="w")
        self.weekly_header.grid(row=0, column=1, padx=10, sticky="w")
    ...
    def populate_task_columns(self):
        ...
        # Subheaders smaller but bold
        subheader_font = ("Segoe UI", 9, "bold")

        for section in DAILY_TASKS:
            ttk.Label(self.daily_col, text=section, font=subheader_font).pack(anchor="w", padx=10, pady=(5, 0))
            for task in DAILY_TASKS[section]:
                if task == "---":
                    ttk.Separator(self.daily_col, orient='horizontal').pack(fill='x', padx=20, pady=4)
                    continue
                if self.visibility_settings[task].get() and not self.checked_tasks[task].get():
                    cb = ttk.Checkbutton(self.daily_col, text=task, variable=self.checked_tasks[task])
                    cb.pack(anchor="w", padx=20)

        for section in WEEKLY_TASKS:
            ttk.Label(self.weekly_col, text=section, font=subheader_font).pack(anchor="w", padx=10, pady=(5, 0))
            for i, task in enumerate(WEEKLY_TASKS[section]):
                if self.visibility_settings[task].get() and not self.checked_tasks[task].get():
                    cb = ttk.Checkbutton(self.weekly_col, text=task, variable=self.checked_tasks[task])
                    cb.pack(anchor="w", padx=20)
                if section == "Quests":
                    if task == "Archon Hunt":
                        ttk.Separator(self.weekly_col, orient='horizontal').pack(fill='x', padx=20, pady=4)
                    elif task == "Netracell":
                        ttk.Separator(self.weekly_col, orient='horizontal').pack(fill='x', padx=20, pady=4)
